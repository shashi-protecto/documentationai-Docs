...
<Steps>
  <Step title="Register the identification endpoint (Protecto API)" icon="settings" titleType="p">
    ...
    **Method:** `PUT`
    **Endpoint:** `{baseurl}/api/vault/metadata/add-or-update/custom-tag/identification-endpoint`
  ...
  <Step title="Register the whitelist endpoint (optional, Protecto API)" icon="check-circle" titleType="p">
    ...
    **Method:** `PUT`
    **Endpoint:** `{baseurl}/api/vault/metadata/add-or-update/custom-tag/pi-list-endpoint`

    <Request tabs="cURL">
      ```bash
      curl -X PUT {baseurl}/api/vault/metadata/add-or-update/custom-tag/pi-list-endpoint \
        -H "Authorization: Bearer YOUR_AUTH_TOKEN" \
        -H "Content-Type: application/json" \
        -d '{
          "data": {
  ...
  <Step title="Verify the tag configuration" icon="check-square" titleType="p">
    ...
    **Method:** `PUT`
    **Endpoint:** `{baseurl}/api/vault/metadata/custom-tag/details`

    <Request tabs="cURL">
      ```bash
      curl -X PUT {baseurl}/api/vault/metadata/custom-tag/details \
        -H "Authorization: Bearer YOUR_AUTH_TOKEN" \
        -H "Content-Type: application/json" \
...
```bash
curl -X PUT {baseurl}/api/vault/metadata/delete/custom-tag \
  -H "Authorization: Bearer YOUR_AUTH_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{ "data": { "tag_type": "CUSTOMER_ID" } }'
```
