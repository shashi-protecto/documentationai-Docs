...
| Method | Endpoint |
|--------|----------|
| `PUT` | `{baseurl}/api/vault/data-scan/pi-elements/update` |
...
```bash
curl -X PUT {baseurl}/api/vault/data-scan/pi-elements/update \
...
| Method | Endpoint |
|--------|----------|
| `PUT` | `{baseurl}/api/vault/data-scan/pi-elements/delete` |
...
```bash
curl -X PUT {baseurl}/api/vault/data-scan/pi-elements/delete \
  -H "Authorization: Bearer <AUTH_TOKEN>" \
  -H "Content-Type: application/json; charset=utf-8" \
  -d '{
    "data_source_name": "salesforce",
