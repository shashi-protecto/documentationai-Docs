...
description: "Poll the status of an async mask or unmask job using a tracking ID, and retrieve results when the job completes."
---
...
| Method | URL |
|--------|-----|
| PUT | `https://protecto-trial.protecto.ai/api/vault/async-status` |

## Request body

<ParamField body="status" param-type="array" required="true">
  List of tracking IDs to check status for.
</ParamField>

<ParamField body="status[].tracking_id" param-type="string" required="true">
...
<ResponseField name="call_type" field-type="string" required="true">
...
<ResponseField name="result" field-type="array" required="false">
  The masking or unmasking output. Only present when `status` is `SUCCESS`.
</ResponseField>

<ResponseField name="error_msg" field-type="string" required="false">
  Error description. Only present when `status` is `FAILED`.
</ResponseField>

<ResponseField name="submitted_time" field-type="string" required="true">
  Timestamp when the job was submitted.
</ResponseField>

<ResponseField name="completed_time" field-type="string" required="false">
